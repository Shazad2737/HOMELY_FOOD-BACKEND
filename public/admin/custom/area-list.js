try {
    ...
    var KTDatatablesServerSide = (function () {
        ...
        var initDatatable = function () {
            dt = $('#area_table').DataTable({
                ...
                pageLength: 50,
                ajax: {
                    url: `/admin/location/area/json`,
                    data: function (d) {
                        d.locationId = $('#area_location_filter').val()
                        d.status = $('#area_status_filter').val()
                    },
                },
                columns: [
                    {
                        ...
                        defaultContent: '<i>Area Name</i>',
                    },
                    {
                        data: 'location.name',
                        className: 'min-w-150px',
                        defaultContent: '<i>No Location</i>',
                    },
                    { data: 'latitude', className: 'min-w-100px' },
                    { data: 'isActive', className: 'min-w-70px' },
                    { data: '_count.users', className: 'min-w-70px' },
                ...
                columnDefs: [
                    {
                        ...
                        render: (data, type, row) => `
                            <div class="d-flex flex-column">
                                <a class="text-gray-800 text-hover-primary fw-bold mb-1" href="/admin/location/area/edit/${
                                    row.id
                                }">
                                    ${row.name || ''}
                                </a>
                    ...
                    },
                    {
                        ...
                        render: (data) => {
                            let badgeClass =
                                areaStatusBadge[data] || 'secondary'
                        ...
                        render: (data, type, row) => `
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 
                                ...
                                <div class="menu-item px-3">
                                    <a href="/admin/location/area/edit/${
                                        row.id
                                ...
                                <div class="menu-item px-3">
                                    <a href="#" class="menu-link px-3 text-danger delete-area-btn" data-id="${
                                        row.id
                                    }">
            ...
            if (locationSelect) {
                fetch('/admin/location/area/locations')
                    .then((res) => res.json())
                    .then((data) => {
                        if (data.success) {
                            data.locations.forEach((loc) => {
                                $(locationSelect).append(
                                    new Option(loc.name, loc.id, false, false)
                                )
                            })
                ...
                $(locationSelect).on('change', () => dt.ajax.reload())
                document
                    .querySelector('#area_location_clear')
            ...
            $('#area_status_filter').on('change', () => dt.ajax.reload())
    ...
    $('#area_table').on('click', '.toggle-status-btn', (e) => {
        e.preventDefault()
        const id = $(e.currentTarget).data('id')
        handleAction({
            url: `/admin/location/area/toggle-status/${id}`,
            confirmTitle: 'Change Status?',
            confirmText: "Are you sure you want to change this area's status?",
            successMessage: 'Area status updated successfully!',
        })
    })

    $('#area_table').on('click', '.delete-area-btn', (e) => {
        e.preventDefault()
        const id = $(e.currentTarget).data('id')
        handleAction({
            url: `/admin/location/area/delete/${id}`,
            method: 'DELETE',
            confirmTitle: 'Delete Area?',
            confirmText: 'This action cannot be undone. Delete this area?',
